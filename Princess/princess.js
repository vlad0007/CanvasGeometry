...
function princess()
{
    ...
    controller = new function() 
	{
		...
		this.R3 = R3;
		this.crown_ang_gd = crown_ang_gd / DEGREE;
		this.pav_ang_gd = pav_ang_gd / DEGREE;
		...
		this.hp = hp;
		this.pavilionAngle = pavilionAngle / DEGREE;
		this.virtCuletOffsetX = virtCuletOffsetX / DEGREE;
		this.virtCuletOffsetY = virtCuletOffsetY / DEGREE;
		this.pavilionFirstLevel = pavilionFirstLevel;
		this.pavilionLastLevel = pavilionLastLevel;
		this.kf_bulge_s2 = kf_bulge_s2;
		this.kf_bulge_s3 = kf_bulge_s3;
		this.kf_flat_s2 = kf_flat_s2;
	...
	f1.add(controller, 'del_gd', 0.0, 0.1).onChange( function() 
	{
		var temp = del_gd;
		del_gd = controller.del_gd;
		recalc();
		if (isCorrect() == -1) 
		{
		   del_gd = temp;
	...
	f1.add(controller, 'crown_ang_gd', 1, 95).onChange( function() 
	{
		var temp = crown_ang_gd;
		crown_ang_gd = (controller.crown_ang_gd) * DEGREE;
		recalc();
		if (isCorrect() == -1) 
		{
		   crown_ang_gd = temp;
		...
		draw();
		controller.crown_ang_gd = crown_ang_gd / DEGREE;
		gui.updateDisplay();
	});
	f1.add(controller, 'pav_ang_gd', 1, 95).onChange( function() 
	{
		var temp = pav_ang_gd;
		pav_ang_gd = (controller.pav_ang_gd) * DEGREE;
		recalc();
		if (isCorrect() == -1) 
		{
		   pav_ang_gd = temp;
		...
		draw();
		controller.pav_ang_gd = pav_ang_gd / DEGREE;
		gui.updateDisplay();
	});
	f1.add(controller, 'ang_corner', 5, 95).onChange( function() 
	{
		var temp = ang_corner;
    ...
    f3.add(controller, 'hp', 0.1, 1.5).onChange( function() 
	{
		var temp = hp;
		hp = controller.hp;
		recalc(); 
    ...
    f3.add(controller, 'pavilionAngle', 10.0, 90.0).onChange( function() 
	{
		var temp = pavilionAngle;
		pavilionAngle = (controller.pavilionAngle) * DEGREE;
		recalc();   
	    if (isCorrect() == -1) 
	    {
		   pavilionAngle = temp;
		...
		draw();
		controller.pavilionAngle = pavilionAngle / DEGREE;
		gui.updateDisplay();
    });
    f3.add(controller, 'virtCuletOffsetX', -10, 10).onChange( function() 
	{
		var temp = virtCuletOffsetX;
		virtCuletOffsetX = (controller.virtCuletOffsetX) * DEGREE;
		recalc(); 
	    if (isCorrect() == -1) 
	    {
		   virtCuletOffsetX = temp;
		   recalc();
	    }
		draw();
		controller.virtCuletOffsetX = virtCuletOffsetX / DEGREE;
		gui.updateDisplay();
    });
    f3.add(controller, 'virtCuletOffsetY', -10.0, 10.0).onChange( function() 
	{
		var temp = virtCuletOffsetY;
		virtCuletOffsetY = (controller.virtCuletOffsetY) * DEGREE;
		recalc(); 
	    if (isCorrect() == -1) 
	    {
		   virtCuletOffsetY = temp;
		   recalc();
	    }
		draw();
		controller.virtCuletOffsetY = virtCuletOffsetY / DEGREE;
		gui.updateDisplay();
    });	
    f3.add(controller, 'pavilionFirstLevel', 0.2, 0.9).onChange( function() 
	{
		var temp = pavilionFirstLevel;
		pavilionFirstLevel = controller.pavilionFirstLevel;
		recalc(); 
	    if (isCorrect() == -1) 
	    {
		   pavilionFirstLevel = temp;
		   recalc();
	    }
		draw();
		controller.pavilionFirstLevel = pavilionFirstLevel;
		gui.updateDisplay();
    });
    f3.add(controller, 'pavilionLastLevel', 0.2, 0.9).onChange( function() 
	{
		var temp = pavilionLastLevel;
		pavilionLastLevel = controller.pavilionLastLevel;
		recalc(); 
	    if (isCorrect() == -1) 
	    {
		   pavilionLastLevel = temp;
		   recalc();
	    }
		draw();
		controller.pavilionLastLevel = pavilionLastLevel;
		gui.updateDisplay();
    });
    f3.add(controller, 'kf_bulge_s2', 0.01, 1.0).onChange( function() 
	{
		var temp = kf_bulge_s2;
		kf_bulge_s2 = controller.kf_bulge_s2;
		recalc(); 
	    if (isCorrect() == -1) 
	    {
		   kf_bulge_s2 = temp;
		...
		draw();
		controller.kf_bulge_s2 = kf_bulge_s2;
		gui.updateDisplay();
    });
    f3.add(controller, 'kf_bulge_s3', 0.01, 0.5).onChange( function() 
	{
		var temp = kf_bulge_s3;
		kf_bulge_s3 = controller.kf_bulge_s3;
		recalc(); 
	    if (isCorrect() == -1) 
	    {
		   kf_bulge_s3 = temp;
		...
		draw();
		controller.kf_bulge_s3 = kf_bulge_s3;
		gui.updateDisplay();
    });
    f3.add(controller, 'kf_flat_s2', 0.01, 1.0).onChange( function() 
	{
		var temp = kf_flat_s2;
		kf_flat_s2 = controller.kf_flat_s2;
		recalc(); 
	    if (isCorrect() == -1) 
	    {
		   kf_flat_s2 = temp;
		...
		draw();
		controller.kf_flat_s2 = kf_flat_s2;
		gui.updateDisplay();
    });
	...
	gui.add( controller, 'correct', true ).onChange( function() 
	{
		...
		if (correct == true)
		{
			...
			del_gd = 0.02;
			rounnd_cir1 = 0.054;         // "Roundness front"
			rounnd_cir2 = 0.054;         // "Roundness flank"
			R3 = 0.11;                   // "Radius corner"
			crown_ang_gd = 35.0*DEGREE;  // то же что и ang_2 и ang_3
			pav_ang_gd = 63.0*DEGREE;    // то же что и ang_2 и ang_3
			ang_corner = 45.0*DEGREE;    // "Corner angle girdle"
			gd_segments = 0.29;          // // Основной параметр задающий размеры сегментов

			// Корона
			beta = 32*DEGREE;            // Угол короны
			t = 0.60;                    // Размер площадки
			starFacets = 0.5;            // Задает положение средних вершин короны

			// Павильон
			hp = 0.57;
			pavilionAngle = 56*DEGREE; // Угол наклона грани павильона примыкающей к рундисту
			virtCuletOffsetX = 0.00001*DEGREE;  // Смещение виртуальной калетты по оси OX
			virtCuletOffsetY = 0.00001*DEGREE;  // Смещение виртуальной калетты по оси OY
			pavilionFirstLevel = 0.65;  // Отношение размера  большой грани 
											//    к максимально-возможному ( >= 0, <= 1 )
			pavilionLastLevel = 0.35;   // Отношение размера основной грани 
											//    к максимально-возможному ( >= 0, <= 1 )
			kf_bulge_s2 = 0.5           // Задает выпуклость грани S2 с выпуклой стороны
			kf_bulge_s3 = 0.1;          // Задает выпуклость грани S3 с выпуклой стороны
			kf_flat_s2 = 0.50;          // Задает выпуклость грани S2 с плоской стороны
			kf_flat_s3 = 0.01;          // Задает выпуклость грани S2 с плоской стороны			
			
			controller.lw = lw;
			controller.r = r;
			controller.del_gd = del_gd;
			controller.rounnd_cir1 = rounnd_cir1;
			controller.rounnd_cir2 = rounnd_cir2;
			controller.R3 = R3;
			controller.crown_ang_gd = crown_ang_gd / DEGREE;
