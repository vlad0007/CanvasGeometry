...
function facet_colors()
{
	var i = 0;
	// table - площадка
	colors[i] = "rgb(100, 100, 130)";
	
	// Корона
	for (i = 1; i < 13; i++)
	{
		colors[i] = "rgb(150, 160, 220)"; // 1
	}	
	
	for (i = 13; i < 25; i++)
	{
		
		colors[i] = "rgb(70, 70, 90)"; // 2
	}

	for (i = 25; i < 37; i++)
	{
		colors[i] = "rgb(150, 150, 180)"; // 3
	}	
	
	for (i = 37; i < 49; i++)
	{
		colors[i] = "rgb(170, 160, 220)";  // 4
	}
	
	for (i = 49; i < 73; i++)
	{
		colors[i] = "rgb(70, 110, 100)"; i++;
		colors[i] = "rgb(50, 90, 120)"; 
	}
	
	//  Рундист
	for (i = 73; i < 169; i++)
	{
		colors[i] = "rgb(100, 100, 140)"; i++;
		colors[i] = "rgb(120, 120, 150)";
	}
	
	// Павильон
	for (i = 169; i < 193; i++)
	{
		colors[i] = "rgb(50, 50, 70)"; i++
		colors[i] = "rgb(80, 80, 100)";		
	}
	
	for (i = 193; i < 205; i++)
	{
		colors[i] = "rgb(90, 90, 120)";
	}	
	
	for (i = 205; i < 217; i++)
	{
		colors[i] = "rgb(110, 110, 150)";
	}	
	
	for (i = 217; i < 229; i++)
	{
		colors[i] = "rgb(30, 30, 120)";
	}	
	
	for (i = 229; i < 241; i++)
	{
		colors[i] = "rgb(110, 40, 180)";
	}
	
	for (i = 241; i < 253; i++)
	{
		colors[i] = "rgb(150, 150, 180)"; i++
		colors[i] = "rgb(190, 190, 220)";
	}		
};

// Проверка выпуклости модели
function isCorrect_Supermultifacet()
{	
	if (correct == false)
	{
		return 1;
