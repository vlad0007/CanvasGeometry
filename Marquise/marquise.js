...
function marquise()
{
    ...
    controller = new function() 
	{
		...
		this.DeltaAngleCrown = DeltaAngleCrown / DEGREE;
		this.t = t;
	...
	gui.add( controller, 'crown_variant_2', false ).onChange( function() 
	{
		crown_variant_2 = controller.crown_variant_2; 
		recalc();
		draw();
	});
    ...
    f2.add(controller, 'DeltaAngleCrown', -10, 10).onChange( function() 
	{
		var temp = DeltaAngleCrown;
		DeltaAngleCrown = (controller.DeltaAngleCrown) * DEGREE;
		recalc();
	    if (isCorrect() == -1) 
	    {
		   DeltaAngleCrown = temp;
		   recalc();
	    }
		draw();
		controller.DeltaAngleCrown = DeltaAngleCrown / DEGREE;
		gui.updateDisplay();
	});	
    ...
    f3.add(controller, 'angle_pav', 30.0, 88.0).onChange( function() 
	{
		var temp = angle_pav;
	...
	gui.add( controller, 'correct', true ).onChange( function() 
	{
		...
		{	
			...
			DeltaAngleCrown = -1.0*DEGREE; // Значение отклонения угла наклона короны
			...
			// Павильон
			angle_pav = 40*DEGREE; // Угол павильона
